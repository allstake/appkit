...
describe('SafeLocalStorage safe', () => {
  ...
  it('should setItem', () => {
    expect(SafeLocalStorage.setItem('@w3mx/wallet_id', 'test')).toBe(undefined)
    expect(setItem).toHaveBeenCalledWith('@w3mx/wallet_id', '"test"')
  })

  it('should getItem', () => {
    expect(SafeLocalStorage.getItem('@w3mx/wallet_id')).toEqual({ test: 'test' })
    expect(getItem).toHaveBeenCalledWith('@w3mx/wallet_id')
  })

  it('should removeItem', () => {
    expect(SafeLocalStorage.removeItem('@w3mx/wallet_id')).toBe(undefined)
    expect(removeItem).toHaveBeenCalledWith('@w3mx/wallet_id')
  })
})
