...
function headings() {
  ...
  const isEmail = connectors.length === 1 && connectors[0]?.id === 'w3mx-email'
  ...
  private dynamicButtonTemplate() {
    ...
    if (isAccountView) {
      return html`<wui-select
        id="dynamic"
        data-testid="w3mx-account-select-network"
