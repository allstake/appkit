...
@customElement('w3mx-connecting-wc-view')
export class W3mConnectingWcView extends LitElement {
  ...
  public override render() {
    if (!this.wallet) {
      return html`<w3mx-connecting-wc-qrcode></w3mx-connecting-wc-qrcode>`
    }
  ...
  private async initializeConnection(retry = false) {
    try {
      if (this.platform === 'browser') {
         ...
         * in this case the connection is handled in w3mx-connecting-wc-browser component.
         */
        return
      }
  ...
  private platformTemplate() {
    switch (this.platform) {
      case 'browser':
        return html`<w3mx-connecting-wc-browser></w3mx-connecting-wc-browser>`
      case 'desktop':
        return html`
          <w3mx-connecting-wc-desktop .onRetry=${() => this.initializeConnection(true)}>
          </w3mx-connecting-wc-desktop>
        `
      case 'web':
        return html`
          <w3mx-connecting-wc-web .onRetry=${() => this.initializeConnection(true)}>
          </w3mx-connecting-wc-web>
        `
      case 'mobile':
        return html`
          <w3mx-connecting-wc-mobile isMobile .onRetry=${() => this.initializeConnection(true)}>
          </w3mx-connecting-wc-mobile>
        `
      case 'qrcode':
        return html`<w3mx-connecting-wc-qrcode></w3mx-connecting-wc-qrcode>`
      default:
        return html`<w3mx-connecting-wc-unsupported></w3mx-connecting-wc-unsupported>`
    }
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'w3mx-connecting-wc-view': W3mConnectingWcView
  }
}
