...
import { getW3mThemeVariables } from '@web3modal-x/common'
...
@customElement('w3mx-approve-transaction-view')
export class W3mApproveTransactionView extends LitElement {
  ...
  private iframe = document.getElementById('w3mx-iframe') as HTMLIFrameElement
    ...
    this.unsubscribe.push(
      ...[
        ...
        ModalController.subscribeKey('shake', val => {
          if (val) {
            this.iframe.style.animation = `w3mx-shake 500ms var(--wui-ease-out-power-2)`
          } else {
  ...
  private onShowIframe() {
    ...
    this.iframe.style.animation = isMobile
      ? 'w3mx-iframe-zoom-in-mobile 200ms var(--wui-ease-out-power-2)'
      : 'w3mx-iframe-zoom-in 200ms var(--wui-ease-out-power-2)'
  }

  private onHideIframe() {
    this.iframe.style.display = 'none'
    this.iframe.style.animation = 'w3mx-iframe-fade-out 200ms var(--wui-ease-out-power-2)'
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'w3mx-approve-transaction-view': W3mApproveTransactionView
  }
}
