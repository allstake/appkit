...
@customElement('w3mx-email-verify-device-view')
export class W3mEmailVerifyDeviceView extends LitElement {
  ...
  public override render() {
    if (!this.email) {
      throw new Error('w3mx-email-verify-device-view: No email provided')
    }
    if (!this.authConnector) {
      throw new Error('w3mx-email-verify-device-view: No auth connector provided')
    }
    ...
    return html`
      ...
      >
        ...
        <wui-flex flexDirection="column" alignItems="center" gap="s">
          ...
          <wui-flex alignItems="center" id="w3mx-resend-section" gap="xs">
    ...
    try {
      if (!this.loading) {
        if (!this.authConnector || !this.email) {
          throw new Error('w3mx-email-login-widget: Unable to resend email')
        }
        this.loading = true
...
declare global {
  interface HTMLElementTagNameMap {
    'w3mx-email-verify-device-view': W3mEmailVerifyDeviceView
  }
}
