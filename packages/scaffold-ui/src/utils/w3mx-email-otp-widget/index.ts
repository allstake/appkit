...
import { W3mFrameHelpers } from '@web3modal-x/wallet'
...
@customElement('w3mx-email-otp-widget')
export class W3mEmailOtpWidget extends LitElement {
  ...
  public override render() {
    if (!this.email) {
      throw new Error('w3mx-email-otp-widget: No email provided')
    }
      ...
      if (this.onOtpResend) {
        if (!this.loading && !this.timeoutTimeLeft) {
          ...
          if (!authConnector || !this.email) {
            throw new Error('w3mx-email-otp-widget: Unable to resend email')
          }
          this.loading = true
...
declare global {
  interface HTMLElementTagNameMap {
    'w3mx-email-otp-widget': W3mEmailOtpWidget
  }
}
